...
exports.getAllBuses = async (req, res) => {
  try {
    const buses = await Bus.find().populate('routeId', 'number start end');
...
exports.addBus = async (req, res) => {
  ...
  try {
    ...
    const bus = new Bus({ ntcNo, busNo, driverId, conductorId, busType, busName, routeId });
    await bus.save();
...
exports.updateOperator = async (req, res) => {
  ...
  try {
    const updatedOperator = await User.findOneAndUpdate({ _id: id, role: 'operator' }, updates, { new: true });
    if (!updatedOperator) {
      return res.status(404).json({ error: 'Operator not found' });
    }
